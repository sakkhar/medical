...
from hospital.forms import LoginForm, AccountRegisterForm, PatientRegisterForm
from hospital.models import Account, Action
...
def setup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            request.session['alert_success'] = "Successfully setup hospital's primary admin account."
            return HttpResponseRedirect('/profile/')
    else:
        form = AccountRegisterForm()
    template_data['form'] = form
    return render(request, 'hospital/setup.html', template_data)
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['alert_success'] = "Successfully logged into hospital."
            return HttpResponseRedirect('/profile/')
    else:
        form = LoginForm()
    template_data['form'] = form
    return render(request, 'hospital/login.html', template_data)
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.log(Action.ACTION_ACCOUNT, "Account login", user.account)
            login(request, user)
            request.session['alert_success'] = "Successfully registered with hospital."
            return HttpResponseRedirect('/profile/')
    else:
        form = PatientRegisterForm()
    template_data['form'] = form
    return render(request, 'hospital/register.html', template_data)
...
def error_denied_view(request):
    ...
    # Proceed with the rest of the view
    return render(request, 'hospital/error/denied.html', template_data)